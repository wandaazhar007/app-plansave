...
export default function DashboardPage() {
  const stats: Stat[] = [
    { label: "Total Balance", value: 12450, change: 2.5, icon: faWallet, kind: "up" },
    { label: "Income", value: 4200, change: 12.3, icon: faArrowTrendUp, kind: "up" },
    { label: "Expenses", value: 2840, change: -3.2, icon: faArrowTrendDown, kind: "down" },
    { label: "Savings", value: 1360, change: 8.1, icon: faDollarSign, kind: "up" },
  ];
          ...
          {recent.map((t) => (
            <div key={t.title + t.meta} className={styles.txRow}>
              ...
              <div className={t.amount >= 0 ? styles.txAmountPos : styles.txAmountNeg}>
                {t.amount >= 0 ? `+${formatUSD(t.amount)}` : `-${formatUSD(Math.abs(t.amount))}`}
              </div>
            </div>
          ))}
        </div>
